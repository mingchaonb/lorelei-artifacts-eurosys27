...
以下数字来自本仓库在第 2 节所述评测机上的 self-hosted CI 运行，该机器为 20 核 ARM64、119 GiB 内存。首次运行需要构建镜像和全部依赖，之后的运行复用 vcpkg 树与已安装的工具，本仓库 CI 所测的就是后者。

| 阶段 | 首次运行 | 复用已装依赖 |
| --- | --- | --- |
| 构建 Ubuntu 24.04 镜像 | 约 5 分钟 | 无 |
| 构建并安装固定版本依赖，7 个工具与 80 个库包 | 约 60 分钟 | 约 30 秒 |
| 安装游戏包 | 约 25 分钟 | 约 5 分钟 |
| 第 1 组，54 个库的正确性 | 约 17 分钟 | 约 17 分钟 |
| 第 2 组，8 个命令行负载、9 条执行路径、每组 5 次 | 约 27 分钟 | 约 27 分钟 |
| 第 3 组，调用与回调开销 breakdown 及覆盖率审计 | 约 1 分钟 | 约 1 分钟 |
| 第 4 组，初始场景游戏帧率 | 约 11 分钟 | 约 11 分钟 |
| 第 5 组，源码改动量分析 | 数秒 | 数秒 |
| 端到端 | 约 2.5 小时 | 约 1 小时 |

上表第 4 组是 CI 可以无人值守运行的初始场景测量。论文报告的帧率来自 3.4 节描述的人工场景导航，耗时更长，且需要有人在 GUI 宿主机前操作。

一份完整的工作区约占 34 GB：

| 路径 | 大小 | 内容 |
| --- | --- | --- |
| `.work/` | 约 16 GB | devkit、各项评测的构建与安装状态、TLC 缓存、绘图虚拟环境 |
| `evaluations/*/results/` | 约 13 GB | 生成的证据，其中命令行那一组约 12 GB 为解码出的媒体文件 |
| `vcpkg/` | 约 5 GB | 源码归档缓存与构建树 |

这三者都位于仓库根目录，且已被 git 忽略，不会进入任何提交。评测结束后可以直接删除以回收空间：
