...
port_dir=$overlay_dir/ports/libcurl
...
work_dir=$repo_root/.work/evaluations/libcurl
...
# Reject incomplete toolkits before creating a misleading evidence directory.
...
# Replace only this recipe's marked scratch directory. Timestamped evidence is
# append-only and is never overwritten.
...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/curl,logs/preparation,logs/native,logs/hecate}
...
# Record enough host and tool identity to distinguish independently reproduced
# runs and preserve the exact invocation accepted by this script.
...
# Count every library-specific TLC input, including the variadic extractor and
# the metadata-only callback anchor manifests.
...
run_logged "$run_dir/logs/preparation/configuration-loc.log" \
    ...
    "$port_dir/lorelei/Manifest_guest.cpp" "$port_dir/lorelei/Manifest_host.cpp"

# Build isolated native, x86_64 guest, and HLR-rewritten host prefixes from the
# same pinned overlay port. vcpkg owns fetching, configuring, and installation.
export LORELEI_DEVKIT=$devkit
export VCPKG_MAX_CONCURRENCY
VCPKG_MAX_CONCURRENCY=$(nproc)
install_lane() {
    local name=$1 triplet=$2 package=libcurl
    if [[ $name == hecate ]]; then package='libcurl[hlr]'; fi
...
stage "Build pinned libcurl packages through the repository overlay"
...
# Copy the exact filtered compilation database and HLR statistics out of the
# package before generating the final two-architecture thunk pack.
hecate_prefix=$work_dir/installed/hecate/$triplet_native
cp -a "$hecate_prefix/share/libcurl/hlr-audit/." "$run_dir/generated/targets/curl/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate libcurl thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libcurl.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name curl --out "$thunk" --lib "$host_library" \
    ...
    --gtl-alias libcurl.so --gtl-alias libcurl.so.4 --no-callback-replace \
    --htl-arg=-fpermissive \
    --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include"

# Install-only still exercises source acquisition, HLR, and thunk generation.
# It records their locations and exits before compiling or running the workload.
if $install_only; then
    python3 - "$run_dir/summary.json" "$hecate_prefix" "$thunk" <<'PY'
import json, pathlib, sys
output, host, thunk = sys.argv[1:]
data = {"schema_version": 2, "package": "libcurl", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "libcurl thunk: $thunk"
    echo "Installation record: $run_dir"
    exit 0
fi

# Compile the same source once for the native host library and once for the
# x86_64 guest thunk. No source-level behavior differs between the lanes.
...
stage "Build the directed loopback HTTP workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -lcurl -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    ...
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libcurl.so -o "$work_dir/tests/guest/test-callbacks"

# Start a private loopback server with a kernel-selected port. It serves the
# same fixed 11-byte body once to each lane and never accesses the network.
stage "Start the fixed loopback HTTP server"
port_file=$work_dir/tests/http-port.txt
python3 "$recipe_dir/tests/http-server.py" --port-file "$port_file" --requests 2 >"$run_dir/logs/preparation/http-server.log" 2>&1 &
server_pid=$!
cleanup_server() {
...
[[ -s $port_file ]] || { echo "Loopback HTTP server did not publish a port" >&2; exit 2; }
url=http://127.0.0.1:$(<"$port_file")/fixed
printf '%s\n' "$url" >"$run_dir/generated/url.txt"

# Run the native control and preserve its output and numeric exit status.
stage "Run the native workload"
set +e
LD_LIBRARY_PATH="$native_prefix/lib" "$work_dir/tests/native/test-callbacks" "$url" 2>&1 | tee "$run_dir/logs/native/callbacks.log"
native_status=${PIPESTATUS[0]}
set -e
printf '%s\n' "$native_status" >"$run_dir/logs/native/exit-status.txt"

# Run the x86_64 program through the Hecate TLC plus HLR path against the same
# server. TLC callback replacement remains explicitly disabled in the thunk.
...
env LORELEI_HOST_EXTENSIONS="$devkit/lib/libLoreHostHLRExtension.so" \
    ...
    "$work_dir/tests/guest/test-callbacks" "$url" 2>&1 | tee "$run_dir/logs/hecate/callbacks.log"
...
# Recompute the final verdict exclusively from committed raw outputs and audit
# JSON so an evaluator can inspect or rerun the classification independently.
