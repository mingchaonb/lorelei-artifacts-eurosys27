...
data = {"schema_version": 2, "experiment_id": run_id, "package": "libpsl", "release": "0.21.5", "result_kind": kind, "mode": "install-only" if install_only == "true" else "test", "mechanism": "TLC Only", "devkit": str(pathlib.Path(devkit).resolve()), "qemu": None if install_only == "true" else str(pathlib.Path(qemu).resolve())}
...
finish_install_only() {
  if $install_only; then
    printf '{"schema_version":2,"package":"libpsl","status":"installed","mode":"install-only","tests_run":false}\n' >"$run_dir/summary.json"
...
ok = native == hecate == "0"
data = {"schema_version": 2, "package": "libpsl", "version": "0.21.5", "mechanism": "TLC Only", "status": "running", "libraries": int(libraries)}
pathlib.Path(out).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
