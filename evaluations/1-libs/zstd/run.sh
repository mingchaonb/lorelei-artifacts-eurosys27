...
for pattern in "${patterns[@]}"; do
  ...
  run_logged "$run_dir/logs/preparation/thunk-$lib_name.log" "$repo_root/evaluations/1-libs/_common/lore-make-thunk.py" "$devkit/bin/LoreMakeThunk.py" --name "$lib_name" --out "$thunk" --lib "$host_lib" --symbols "$audit/Symbols.conf" --desc "$overlay_dir/ports/zstd/lorelei/Desc.h" --gtl-alias "$(basename "$host_lib")" --devkit "$devkit" --keep-intermediates -- -DZDICT_STATIC_LINKING_ONLY -I"$hecate_prefix/include" -I"$hecate_prefix/tools/zstd/upstream-tests/lib"
...
if $install_only; then
  printf '{"schema_version":2,"package":"zstd","status":"installed","mode":"install-only","tests_run":false,"installed_lanes":["native","guest","hecate"],"thunks_generated":true}\n' >"$run_dir/summary.json"
