...
port 构建并安装完整的 37-check 上游测试。`run.sh` 只使用安装后的 package，运行 native 与 Hecate，绝不运行纯 QEMU lane。两条 lane 均以状态 0 退出且输出等价即为成功。`--install-only` 在构建与 audit 后停止，`--reference` 写入只追加证据。
