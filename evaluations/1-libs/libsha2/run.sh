...
work=$repo_root/.work/evaluations/libsha2
...
install_lane() {
    ...
    "$vcpkg" install "libsha2:$triplet" --overlay-ports="$overlay/ports" --overlay-triplets="$overlay/triplets" --x-install-root="$work/installed/$lane" --x-buildtrees-root="$work/vcpkg/$lane/buildtrees" --x-packages-root="$work/vcpkg/$lane/packages" --downloads-root="$repo_root/vcpkg/downloads" >"$run_dir/logs/preparation/vcpkg-$lane.log" 2>&1
}
...
meta = {"schema_version": 2, "package": "libsha2", "release": "snapshot 565f650", "mechanism": "TLC Only", "workload": "37 upstream SHA-256 checks"}
pathlib.Path(sys.argv[1]).write_text(json.dumps(meta, indent=2, sort_keys=True) + "\n")
summary = {"schema_version": 2, "package": "libsha2", "status": "installed", "tests_run": False}
...
data.update({"schema_version": 2, "package": "libsha2", "tests_run": True})
