...
IFS=';' read -r -a target_specs <<< 'opus|libopus.so.*|Symbols.conf|libopus.so,libopus.so.0'
...
if $install_only; then
    ...
    status=pass
    "$repo_root/evaluations/1-libs/_common/audio-signal-upstream.sh" \
        "opus" "$repo_root" "$work" "$run_dir" "$devkit" "$qemu" \
...
if ! $install_only; then echo "ALL TESTS PASSED: opus native + Hecate, pure QEMU not run"; fi
