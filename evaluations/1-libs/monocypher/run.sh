...
work=$repo_root/.work/evaluations/monocypher
...
install_lane() {
    ...
    cmake -E remove_directory "$work/installed/$lane"
...
meta = {"schema_version": 2, "package": "monocypher", "release": "4.0.3", "mechanism": "TLC Only", "workload": "the upstream test.out vector suite"}
pathlib.Path(sys.argv[1]).write_text(json.dumps(meta, indent=2, sort_keys=True) + "\n")
summary = {"schema_version": 2, "package": "monocypher", "status": "installed", "tests_run": False}
...
data.update({"schema_version": 2, "package": "monocypher", "tests_run": True})
