...
port_dir=$repo_root/vcpkg-overlay/ports/libvorbisfile
reference=false
install_only=false
verbose=false
positional=()
...
work=$repo_root/.work/evaluations/libvorbisfile
...
sha256sum "$recipe_dir/tests/Test.c" "$recipe_dir/run.sh" \
    "$repo_root/evaluations/1-libs/_common/audio-signal-upstream.sh" \
    "$repo_root/evaluations/1-libs/_common/audio-signal-ctest.py" >"$run_dir/generated/evaluation-inputs.sha256"
...
install_lane() {
    ...
    run_logged "$run_dir/logs/vcpkg-$lane.log" "$vcpkg" install "libvorbisfile:$triplet"         --overlay-ports="$repo_root/vcpkg-overlay/ports"         --overlay-triplets="$repo_root/vcpkg-overlay/triplets"         --x-install-root="$work/installed/$lane"         --x-buildtrees-root="$work/vcpkg/$lane/buildtrees"         --x-packages-root="$work/vcpkg/$lane/packages"         --downloads-root="$repo_root/vcpkg/downloads" --triplet="$triplet" --binarysource=clear
}
...
IFS=';' read -r -a target_specs <<< 'vorbisfile|libvorbisfile.so.*|Symbols.conf|libvorbisfile.so,libvorbisfile.so.3'
...
if $install_only; then
    ...
    status=pass
    "$repo_root/evaluations/1-libs/_common/audio-signal-upstream.sh" \
        "libvorbisfile" "$repo_root" "$work" "$run_dir" "$devkit" "$qemu" \
...
if ! $install_only; then echo "ALL TESTS PASSED: libvorbisfile native + Hecate, pure QEMU not run"; fi
