...
port_dir=$overlay_dir/ports/libxml2
...
work_dir=$repo_root/.work/evaluations/libxml2
...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/xml2,logs/preparation,logs/native,logs/hecate}
...
# Count the complete repository-owned TLC API configuration.
...
install_lane() {
    local name=$1 triplet=$2 package=libxml2
    if [[ $name == hecate ]]; then package='libxml2[hlr]'; fi
...
stage "Build pinned libxml2 packages through the repository overlay"
...
cp -a "$hecate_prefix/share/libxml2/hlr-audit/." "$run_dir/generated/targets/xml2/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate libxml2 thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libxml2.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name xml2 --out "$thunk" --lib "$host_library" \
    --symbols "$port_dir/lorelei/Symbols.conf" --desc "$port_dir/lorelei/Desc.h" \
    --gtl-alias libxml2.so --gtl-alias libxml2.so.16 --no-callback-replace \
    --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include/libxml2"

# Install-only records the completed package and thunk without running the workload.
if $install_only; then
    python3 - "$run_dir/summary.json" "$hecate_prefix" "$thunk" <<'PY'
import json, pathlib, sys
output, host, thunk = sys.argv[1:]
data = {"schema_version": 2, "package": "libxml2", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "libxml2 thunk: $thunk"
    echo "Installation record: $run_dir"
    exit 0
fi

# Compile the identical push-parser workload for the native and guest lanes.
...
stage "Build the directed SAX push-parser workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include/libxml2" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -lxml2 -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    --sysroot="$devkit/x86_64/sysroot" -I"$guest_prefix/include/libxml2" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libxml2.so -o "$work_dir/tests/guest/test-callbacks"

# Run the native control and preserve raw output plus its numeric exit status.
...
# Run the x86_64 program through Hecate with callback replacement disabled.
...
# Recompute the verdict from raw logs and the committed HLR audit.
