...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/pcap,logs/preparation,logs/native,logs/hecate}
...
install_lane() {
    local name=$1 triplet=$2 package=libpcap
    if [[ $name == hecate ]]; then package='libpcap[hlr]'; fi
...
stage "Build pinned libpcap packages through the repository overlay"
...
cp -a "$hecate_prefix/share/libpcap/hlr-audit/." "$run_dir/generated/targets/pcap/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate libpcap thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libpcap.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name pcap --out "$thunk" --lib "$host_library" \
    --symbols "$port_dir/lorelei/Symbols.conf" --desc "$port_dir/lorelei/Desc.h" \
    --gtl-alias libpcap.so --gtl-alias libpcap.so.1 --no-callback-replace \
...
data = {"schema_version": 2, "package": "libpcap", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "libpcap thunk: $thunk"
...
input=$run_dir/generated/one-packet.pcap
xxd -r -p "$recipe_dir/tests/one-packet.hex" "$input"
sha256sum "$input" >"$run_dir/generated/one-packet.sha256"
stage "Build the directed offline-capture workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -lpcap -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    ...
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libpcap.so -o "$work_dir/tests/guest/test-callbacks"

stage "Run the native workload"
set +e
LD_LIBRARY_PATH="$native_prefix/lib" "$work_dir/tests/native/test-callbacks" "$input" 2>&1 | tee "$run_dir/logs/native/callbacks.log"
...
env LORELEI_HOST_EXTENSIONS="$devkit/lib/libLoreHostHLRExtension.so" \
    ...
    "$work_dir/tests/guest/test-callbacks" "$input" 2>&1 | tee "$run_dir/logs/hecate/callbacks.log"
