...
port_dir=$overlay_dir/ports/libuv
...
work_dir=$repo_root/.work/evaluations/libuv
...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/uv,logs/preparation,logs/native,logs/hecate}
...
install_lane() {
    local name=$1 triplet=$2 package=libuv
    if [[ $name == hecate ]]; then package='libuv[hlr]'; fi
...
stage "Build pinned libuv packages through the repository overlay"
...
cp -a "$hecate_prefix/share/libuv/hlr-audit/." "$run_dir/generated/targets/uv/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate libuv thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libuv.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name uv --out "$thunk" --lib "$host_library" \
    --symbols "$port_dir/lorelei/Symbols.conf" --desc "$port_dir/lorelei/Desc.h" \
    --gtl-alias libuv.so --gtl-alias libuv.so.1 --no-callback-replace \
...
data = {"schema_version": 2, "package": "libuv", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "libuv thunk: $thunk"
    echo "Installation record: $run_dir"
    exit 0
fi

# Compile the identical threadpool workload for the native and guest lanes.
...
stage "Build the directed threadpool callback workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -luv -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    ...
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libuv.so -o "$work_dir/tests/guest/test-callbacks"
...
# Run the x86_64 program through Hecate with the thread hook and HLR extensions.
