...
port_dir=$overlay_dir/ports/libtommath
...
work_dir=$repo_root/.work/evaluations/libtommath
...
# Validate the devkit before creating a result that could be mistaken for a
...
# Replace only this recipe's marked scratch directory and keep timestamped
# result directories append-only.
...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/tommath,logs/preparation,logs/native,logs/hecate}
...
# Preserve the invocation, host environment, vcpkg version, HLR version, and
# QEMU identity needed to distinguish independently reproduced runs.
...
# Count the complete per-library API description and symbol selection rather
# than counting generated HLR source.
...
# Build isolated native, x86_64 guest, and HLR-rewritten host installations
# from the same official archive and overlay port.
export LORELEI_DEVKIT=$devkit
export VCPKG_MAX_CONCURRENCY
VCPKG_MAX_CONCURRENCY=$(nproc)
install_lane() {
    local name=$1 triplet=$2 package=libtommath
    if [[ $name == hecate ]]; then package='libtommath[hlr]'; fi
...
stage "Build pinned LibTomMath packages through the repository overlay"
...
# Copy the filtered compilation database and HLR statistics out of the package
# before generating the final guest and host thunk pair.
hecate_prefix=$work_dir/installed/hecate/$triplet_native
cp -a "$hecate_prefix/share/libtommath/hlr-audit/." "$run_dir/generated/targets/tommath/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate LibTomMath thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libtommath.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name tommath --out "$thunk" --lib "$host_library" \
    --symbols "$port_dir/lorelei/Symbols.conf" --desc "$port_dir/lorelei/Desc.h" \
    --gtl-alias libtommath.so --gtl-alias libtommath.so.1 --no-callback-replace \
    --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include"

# Install-only includes official source acquisition, HLR, package installation,
# and thunk generation, then records their locations without running tests.
if $install_only; then
    python3 - "$run_dir/summary.json" "$hecate_prefix" "$thunk" <<'PY'
import json, pathlib, sys
output, host, thunk = sys.argv[1:]
data = {"schema_version": 2, "package": "libtommath", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "LibTomMath thunk: $thunk"
    echo "Installation record: $run_dir"
    exit 0
fi

# Compile the same directed callback workload for the host control and x86_64
# guest. Both dynamically link the package generated in their own lane.
...
stage "Build the persistent random-source callback workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -ltommath -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    ...
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libtommath.so -o "$work_dir/tests/guest/test-callbacks"

# Run and record the native control before entering the pass-through path.
...
# Run the x86_64 workload through QEMU with both HLR extensions and the thread
# hook. TLC callback replacement remains disabled.
...
# Recompute the verdict from raw lane output and audit JSON. The two products
# must match exactly in addition to both callbacks being invoked twice.
