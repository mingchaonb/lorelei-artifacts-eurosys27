...
port_dir=$repo_root/vcpkg-overlay/ports/fftw3
reference=false
install_only=false
verbose=false
positional=()
...
work=$repo_root/.work/evaluations/fftw3
...
install_lane() {
    ...
    run_logged "$run_dir/logs/vcpkg-$lane.log" "$vcpkg" install "fftw3:$triplet"         --overlay-ports="$repo_root/vcpkg-overlay/ports"         --overlay-triplets="$repo_root/vcpkg-overlay/triplets"         --x-install-root="$work/installed/$lane"         --x-buildtrees-root="$work/vcpkg/$lane/buildtrees"         --x-packages-root="$work/vcpkg/$lane/packages"         --downloads-root="$repo_root/vcpkg/downloads" --triplet="$triplet" --binarysource=clear
}
...
IFS=';' read -r -a target_specs <<< 'fftw3|libfftw3.so.*|Symbols.conf|libfftw3.so,libfftw3.so.3'
...
if $install_only; then
    ...
    status=pass
    "$repo_root/evaluations/1-libs/_common/audio-signal-upstream.sh" \
        "fftw3" "$repo_root" "$work" "$run_dir" "$devkit" "$qemu" \
...
if ! $install_only; then echo "ALL TESTS PASSED: fftw3 native + Hecate, pure QEMU not run"; fi
