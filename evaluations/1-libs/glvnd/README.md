...
The game path retains two explicit adaptations. When Unity requests extended context attributes, the host manifest creates the context through the system GLVND entry. GLX proc addresses returned by a preloaded MangoHud are mapped back to the GL thunk in `ThunkDB.json`, so the dynamic proc-address path still reaches the same Hecate graphics interface.
