...
游戏路径还保留两项明确适配。Unity 请求 extended context attributes 时，host manifest 通过系统 GLVND 入口创建 context。MangoHud preload 返回的 GLX proc 地址在 `ThunkDB.json` 中映射回 GL thunk，使动态 proc-address 路径仍能调用相同的 Hecate 图形接口。
