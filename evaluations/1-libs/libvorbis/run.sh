...
port_dir=$repo_root/vcpkg-overlay/ports/libvorbis
reference=false
install_only=false
verbose=false
positional=()
...
IFS=';' read -r -a target_specs <<< 'ogg|libogg.so.*|ogg.Symbols.conf|libogg.so,libogg.so.0;vorbis|libvorbis.so.*|vorbis.Symbols.conf|libvorbis.so,libvorbis.so.0;vorbisenc|libvorbisenc.so.*|vorbisenc.Symbols.conf|libvorbisenc.so,libvorbisenc.so.2'
