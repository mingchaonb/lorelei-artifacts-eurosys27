...
```bash
./evaluations/1-libs/sdl2-ttf/run.sh --reference --verbose /path/to/lorelei-devkit
```
...
Run `./evaluations/1-libs/sdl2-ttf/run.sh /path/to/lorelei-devkit` to install both architectures, generate the TLC thunks, and run the directed test. Pass `--install-only` to stop after package installation, HLR audit collection, and thunk generation. This directed workload does not claim an upstream all-tests result, so this README does not carry the all-tests marker.
