...
suite_native=$native_prefix/tools/cjson/upstream-tests
suite_guest=$guest_prefix/tools/cjson/upstream-tests
mapfile -t upstream_tests < <(find "$suite_guest" -maxdepth 1 -type f -perm -111 -printf '%f\n' | sort)
[[ ${#upstream_tests[@]} == 19 ]] || { echo "Expected 19 installed cJSON tests" >&2; exit 1; }
for test_name in "${upstream_tests[@]}"; do
  "$nm_tool" -D --undefined-only --just-symbol-name "$suite_guest/$test_name" | sed 's/@.*//' >>"$run_dir/generated/guest-undefined.txt"
done
...
for pattern in "${patterns[@]}"; do
  ...
  run_logged "$run_dir/logs/preparation/thunk-$lib_name.log" "$repo_root/evaluations/1-libs/_common/lore-make-thunk.py" "$devkit/bin/LoreMakeThunk.py" --name "$lib_name" --out "$thunk" --lib "$host_lib" --symbols "$audit/Symbols.conf" --desc "$overlay_dir/ports/cjson/lorelei/Desc.h" --gtl-alias "$(basename "$host_lib")" --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include" -I"$hecate_prefix/include/cjson"
...
ok = native == hecate == "0"
data = {"schema_version": 2, "package": "cjson", "version": "1.7.19", "mechanism": "TLC Only", "status": "running", "libraries": int(libraries)}
pathlib.Path(out).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
...
for lane in native hecate; do
  ...
  for test_name in "${upstream_tests[@]}"; do
    ...
    if [[ $lane == native ]]; then
      (cd "$suite_native" && env LD_LIBRARY_PATH="$native_prefix/lib" "./$test_name") >>"$output" 2>&1
    else
      (cd "$suite_guest" && env LD_LIBRARY_PATH="$devkit/lib:$hecate_prefix/lib:$host_path" "$qemu" -L "$devkit/x86_64/sysroot" -E LD_BIND_NOW=1 -E "LD_LIBRARY_PATH=$devkit/x86_64/lib:$guest_path" "./$test_name") >>"$output" 2>&1
    fi
    echo "PASS $test_name" >>"$output"
  done
  sed -E 's#^/[^:]+/tests/#<SOURCE>/tests/#; s/0x[0-9a-fA-F]+/<PTR>/g; s/[0-9]+ ms/<TIME> ms/g' "$output" >"$run_dir/logs/$lane/upstream-normalized.log"
done
...
data["upstream"] = {"tests": 19, "native_exit_status": 0, "hecate_exit_status": 0, "output_match": True, "installed_by_vcpkg": True}
