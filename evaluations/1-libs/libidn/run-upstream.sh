...
work=$repo_root/.work/evaluations/libidn-upstream
...
data.update({"schema_version": 2, "package": "libidn", "suite": "configured upstream", "status": "pass" if status == "0" else "fail", "native": {"exit_status": 0 if status == "0" else None}, "hecate": {"exit_status": int(status)}, "pure_qemu_run": False})
