...
work=$repo_root/.work/evaluations/libidn
...
data = {"schema_version": 2, "experiment_id": run_id, "package": "libidn", "release": "1.43", "result_kind": kind, "mode": "install-only" if install_only == "true" else "test", "mechanism": "TLC Only", "devkit": str(pathlib.Path(devkit).resolve()), "qemu": None if install_only == "true" else str(pathlib.Path(qemu).resolve())}
...
if $install_only; then
  printf '{"schema_version":2,"package":"libidn","status":"installed","mode":"install-only","tests_run":false}\n' >"$run_dir/summary.json"
  exit 0
fi
...
run_logged "$run_dir/logs/preparation/test-native.log" cc -I"$native_prefix/include" "$recipe_dir/tests/workload.c" -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -lidn -o "$work/tests/native/workload"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" --sysroot="$devkit/x86_64/sysroot" -I"$guest_prefix/include" "$recipe_dir/tests/workload.c" -L"$guest_prefix/lib" -Wl,-rpath,"$guest_prefix/lib" -lidn -o "$work/tests/guest/workload"
"$nm_tool" -D --undefined-only --just-symbol-name "$work/tests/guest/workload" | sed 's/@.*//' | sort -u >"$run_dir/generated/guest-undefined.txt"
suite_native=$native_prefix/tools/libidn/upstream-tests
suite_guest=$guest_prefix/tools/libidn/upstream-tests
mapfile -t upstream_tests < <(find "$suite_guest/bin" -maxdepth 1 -type f -perm -111 -printf '%f\n' | sort)
[[ ${#upstream_tests[@]} == 17 ]] || { echo "Expected 17 installed libidn tests" >&2; exit 1; }
for test_name in "${upstream_tests[@]}"; do "$nm_tool" -D --undefined-only --just-symbol-name "$suite_guest/bin/$test_name" | sed 's/@.*//' >>"$run_dir/generated/guest-undefined.txt"; done
sort -u -o "$run_dir/generated/guest-undefined.txt" "$run_dir/generated/guest-undefined.txt"
thunk_host=()
thunk_guest=()
index=0
IFS=, read -ra patterns <<<"idn.so*"
for pattern in "${patterns[@]}"; do
  ...
  run_logged "$run_dir/logs/preparation/thunk-$lib_name.log" "$devkit/bin/LoreMakeThunk.py" --name "$lib_name" --out "$thunk" --lib "$host_lib" --symbols "$audit/Symbols.conf" --desc "$overlay_dir/ports/libidn/lorelei/Desc.h" --gtl-alias "$(basename "$host_lib")" --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include" -I"$recipe_dir/upstream/include"
...
ok = native == hecate == "0"
data = {"schema_version": 2, "package": "libidn", "version": "1.43", "mechanism": "TLC Only", "status": "pass" if ok else "fail", "libraries": int(libraries), "native": {"exit_status": int(native)}, "hecate": {"exit_status": int(hecate)}, "output_match": True}
pathlib.Path(out).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
...
for lane in native hecate; do
  ...
  for test_name in "${upstream_tests[@]}"; do
    ...
    if [[ $lane == native ]]; then
      (cd "$suite/data" && env LD_LIBRARY_PATH="$native_prefix/lib" "$suite/bin/$test_name") >>"$output" 2>&1
    else
      (cd "$suite/data" && env LD_LIBRARY_PATH="$devkit/lib:$hecate_prefix/lib:$host_path:$work/thunk-libc-shim" "$qemu" -L "$devkit/x86_64/sysroot" -E LD_BIND_NOW=1 -E "LD_PRELOAD=$work/thunk-libc-shim/x86_64/libc-shim.so" -E "LD_LIBRARY_PATH=$devkit/x86_64/lib:$guest_path:$work/thunk-libc-shim/x86_64" "$suite/bin/$test_name") >>"$output" 2>&1
    fi
...
data["upstream"] = {"tests": 17, "native_exit_status": 0, "hecate_exit_status": 0, "output_match": True, "installed_by_vcpkg": True}
