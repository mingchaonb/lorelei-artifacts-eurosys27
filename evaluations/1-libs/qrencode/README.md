...
Run `./evaluations/1-libs/qrencode/run.sh /path/to/lorelei-devkit` to install both architectures, generate the TLC thunk, and run all 12 upstream tests in native and Hecate lanes. The vcpkg port installs all test binaries, helper DSOs, internal test headers, and frame input under `tools/qrencode/upstream-tests`. Its source patches keep allocations and `errno` observations on the host-library side where required by the cross-architecture ABI. Pass `--install-only` to stop after package installation and thunk generation.
