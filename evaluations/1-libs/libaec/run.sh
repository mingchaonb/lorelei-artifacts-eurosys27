...
port_dir=$repo_root/vcpkg-overlay/ports/libaec
reference=false
install_only=false
verbose=false
positional=()
...
work=$repo_root/.work/evaluations/libaec
...
install_lane() {
    ...
    run_logged "$run_dir/logs/vcpkg-$lane.log" "$vcpkg" install "libaec:$triplet"         --overlay-ports="$repo_root/vcpkg-overlay/ports"         --overlay-triplets="$repo_root/vcpkg-overlay/triplets"         --x-install-root="$work/installed/$lane"         --x-buildtrees-root="$work/vcpkg/$lane/buildtrees"         --x-packages-root="$work/vcpkg/$lane/packages"         --downloads-root="$repo_root/vcpkg/downloads" --triplet="$triplet" --binarysource=clear
}
...
IFS=';' read -r -a target_specs <<< 'aec|libaec.so.*|aec.Symbols.conf|libaec.so,libaec.so.0;sz|libsz.so.*|sz.Symbols.conf|libsz.so,libsz.so.2'
...
if $install_only; then
    ...
    status=pass
    "$repo_root/evaluations/1-libs/_common/audio-signal-upstream.sh" \
        "libaec" "$repo_root" "$work" "$run_dir" "$devkit" "$qemu" \
...
if ! $install_only; then echo "ALL TESTS PASSED: libaec native + Hecate, pure QEMU not run"; fi
