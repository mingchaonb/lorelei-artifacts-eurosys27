...
work=$repo_root/.work/evaluations/libmd
...
install_lane() {
    ...
    "$vcpkg" install "libmd:$triplet" --overlay-ports="$overlay/ports" --overlay-triplets="$overlay/triplets" --x-install-root="$work/installed/$lane" --x-buildtrees-root="$work/vcpkg/$lane/buildtrees" --x-packages-root="$work/vcpkg/$lane/packages" --downloads-root="$repo_root/vcpkg/downloads" >"$run_dir/logs/preparation/vcpkg-$lane.log" 2>&1
}
...
meta = {"schema_version": 2, "package": "libmd", "release": "1.2.0", "mechanism": "TLC Only", "workload": "seven digest executables"}
pathlib.Path(sys.argv[1]).write_text(json.dumps(meta, indent=2, sort_keys=True) + "\n")
summary = {"schema_version": 2, "package": "libmd", "status": "installed", "tests_run": False}
...
if ! $install_only; then
    LORELEI_DEVKIT="$devkit" QEMU="$qemu" WORK="$work/upstream" \
        ...
        BENCH="$overlay/ports/libmd/lorelei" "$recipe_dir/tests/run-installed.sh"
...
data.update({"schema_version": 2, "package": "libmd", "tests_run": True})
