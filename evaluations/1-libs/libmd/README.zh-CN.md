...
runner 从 `LORELEI_DEVKIT` 读取 devkit，默认使用 `.work/devkit`。`--install-only` 在两个共享包构建并 audit 后停止，`--reference` 写入只追加证据。生成的 audit 记录 DATA、TLS、allocator ownership、callback、errno、symbol version、SONAME 和动态依赖，即使某类没有 workload hit。
