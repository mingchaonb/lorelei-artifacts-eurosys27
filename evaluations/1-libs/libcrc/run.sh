...
state=$repo_root/.work/evaluations/libcrc
work=$state/run
...
data = {"schema_version": 2, "experiment_id": run_id, "package": "libcrc", "release": "2.0", "result_kind": kind, "mode": "install-only" if install_only == "true" else "test", "mechanism": "TLC Only", "devkit": str(pathlib.Path(devkit).resolve()), "qemu": None if install_only == "true" else str(pathlib.Path(qemu).resolve())}
...
guest_test=$guest_tests/libcrc_testall
...
for pattern in "${patterns[@]}"; do
  ...
  run_logged "$run_dir/logs/preparation/thunk-$lib_name.log" "$devkit/bin/LoreMakeThunk.py" --name "$lib_name" --out "$thunk" --lib "$host_lib" --symbols "$audit/Symbols.conf" --desc "$overlay_dir/ports/libcrc/lorelei/Desc.h" --gtl-alias "$(basename "$host_lib")" --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include"
...
set +e
LD_LIBRARY_PATH="$native_prefix/lib" "$native_test" 2>&1 | tee "$run_dir/logs/native/upstream-testall.log"
native_status=${PIPESTATUS[0]}
set -e
printf '%s\n' "$native_status" >"$run_dir/logs/native/exit-status.txt"
set +e
env LD_LIBRARY_PATH="$devkit/lib:$hecate_prefix/lib:$host_path" "$qemu" -L "$devkit/x86_64/sysroot" -E LD_BIND_NOW=1 -E "LD_LIBRARY_PATH=$devkit/x86_64/lib:$guest_path" "$guest_test" 2>&1 | tee "$run_dir/logs/hecate/upstream-testall.log"
hecate_status=${PIPESTATUS[0]}
set -e
printf '%s\n' "$hecate_status" >"$run_dir/logs/hecate/exit-status.txt"
sed '/no version information available/d' "$run_dir/logs/native/upstream-testall.log" >"$run_dir/logs/native/upstream-testall.normalized"
sed '/no version information available/d' "$run_dir/logs/hecate/upstream-testall.log" >"$run_dir/logs/hecate/upstream-testall.normalized"
cmp "$run_dir/logs/native/upstream-testall.normalized" "$run_dir/logs/hecate/upstream-testall.normalized"
