...
port_dir=$repo_root/vcpkg-overlay/ports/soxr
reference=false
install_only=false
verbose=false
positional=()
...
work=$repo_root/.work/evaluations/soxr
...
install_lane() {
    ...
    run_logged "$run_dir/logs/vcpkg-$lane.log" "$vcpkg" install "soxr:$triplet"         --overlay-ports="$repo_root/vcpkg-overlay/ports"         --overlay-triplets="$repo_root/vcpkg-overlay/triplets"         --x-install-root="$work/installed/$lane"         --x-buildtrees-root="$work/vcpkg/$lane/buildtrees"         --x-packages-root="$work/vcpkg/$lane/packages"         --downloads-root="$repo_root/vcpkg/downloads" --triplet="$triplet" --binarysource=clear
}
...
IFS=';' read -r -a target_specs <<< 'soxr|libsoxr.so.*|Symbols.conf|libsoxr.so,libsoxr.so.0'
...
if ! $install_only; then echo "ALL TESTS PASSED: soxr native + Hecate, pure QEMU not run"; fi
