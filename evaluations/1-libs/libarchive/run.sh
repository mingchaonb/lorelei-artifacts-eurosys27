...
port_dir=$overlay_dir/ports/libarchive
...
work_dir=$repo_root/.work/evaluations/libarchive
...
mkdir -p "$work_dir" "$run_dir"/{generated/targets/archive,logs/preparation,logs/native,logs/hecate}
...
install_lane() {
    local name=$1 triplet=$2 package=libarchive
    if [[ $name == hecate ]]; then package='libarchive[hlr]'; fi
...
stage "Build pinned libarchive packages through the repository overlay"
...
cp -a "$hecate_prefix/share/libarchive/hlr-audit/." "$run_dir/generated/targets/archive/"
cp "$port_dir/lorelei/Symbols.conf" "$run_dir/generated/Symbols.conf"

stage "Generate the Hecate libarchive thunk"
host_library=$(find "$hecate_prefix/lib" -maxdepth 1 -type f -name 'libarchive.so.*' | head -1)
thunk=$work_dir/thunks/hecate
run_logged "$run_dir/logs/preparation/thunk-hecate.log" "$devkit/bin/LoreMakeThunk.py" \
    --name archive --out "$thunk" --lib "$host_library" \
    --symbols "$port_dir/lorelei/Symbols.conf" --desc "$port_dir/lorelei/Desc.h" \
    --gtl-alias libarchive.so --gtl-alias libarchive.so.13 --no-callback-replace \
...
data = {"schema_version": 2, "package": "libarchive", "status": "installed", "mode": "install-only", "tests_run": False, "installed": {"hecate_host": host, "thunk": thunk}}
pathlib.Path(output).write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
    echo "Hecate host library: $hecate_prefix"
    echo "libarchive thunk: $thunk"
    echo "Installation record: $run_dir"
    exit 0
fi

# Compile the identical in-memory tar workload for native and guest lanes.
...
stage "Build the directed archive callback workload"
run_logged "$run_dir/logs/preparation/test-native.log" cc \
    -I"$native_prefix/include" "$recipe_dir/tests/TestCallbacks.c" \
    -L"$native_prefix/lib" -Wl,-rpath,"$native_prefix/lib" -larchive -o "$work_dir/tests/native/test-callbacks"
run_logged "$run_dir/logs/preparation/test-guest.log" "$devkit/bin/x86_64-linux-gnu-clang" \
    ...
    -L"$thunk/x86_64" -Wl,-rpath,"$thunk/x86_64" -l:libarchive.so -o "$work_dir/tests/guest/test-callbacks"
