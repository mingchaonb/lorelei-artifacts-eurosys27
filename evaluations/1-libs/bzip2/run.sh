...
state=$repo_root/.work/evaluations/bzip2
work=$state/run
...
data = {"schema_version": 2, "experiment_id": run_id, "package": "bzip2", "release": "1.0.8", "result_kind": kind, "mode": "install-only" if install_only == "true" else "test", "mechanism": "TLC Only", "devkit": str(pathlib.Path(devkit).resolve()), "qemu": None if install_only == "true" else str(pathlib.Path(qemu).resolve())}
...
if $install_only; then
  printf '{"schema_version":2,"package":"bzip2","status":"installed","mode":"install-only","tests_run":false}\n' >"$run_dir/summary.json"
  exit 0
fi
native_prefix=$state/installed/native/arm64-linux-ae
guest_prefix=$state/installed/guest/x64-linux-ae
native_cli=$native_prefix/tools/bzip2/upstream-tests/bzip2_upstream_test
guest_cli=$guest_prefix/tools/bzip2/upstream-tests/bzip2_upstream_test
chmod +x "$native_cli" "$guest_cli"
"$nm_tool" -D --undefined-only --just-symbol-name "$guest_cli" | sed 's/@.*//' | sort -u >"$run_dir/generated/guest-undefined.txt"
thunk_host=()
thunk_guest=()
index=0
IFS=, read -ra patterns <<<"bz2.so*"
...
run_bzip2_suite() {
  ...
  mkdir -p "$suite_dir"
  cp "$prefix/tools/bzip2/upstream-tests/"sample*.ref "$prefix/tools/bzip2/upstream-tests/"sample*.bz2 "$suite_dir/"
  if [[ $lane == native ]]; then
    command=(env LD_LIBRARY_PATH="$native_prefix/lib" "$native_cli")
  else
    command=(env LD_LIBRARY_PATH="$devkit/lib:$hecate_prefix/lib:$host_path" "$qemu" -L "$devkit/x86_64/sysroot" -E LD_BIND_NOW=1 -E "LD_LIBRARY_PATH=$devkit/x86_64/lib:$guest_path" "$guest_cli")
  fi
  (
    cd "$suite_dir"
