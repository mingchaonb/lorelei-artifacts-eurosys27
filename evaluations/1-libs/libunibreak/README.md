...
Run `./evaluations/1-libs/libunibreak/run.sh /path/to/lorelei-devkit` to install both architectures, generate the TLC thunk, and run all three modes of the upstream test program in native and Hecate lanes. The vcpkg port installs the program and three conformance vectors under `tools/libunibreak/upstream-tests`. Pass `--install-only` to stop after package installation and thunk generation.
