...
work=$repo_root/.work/evaluations/libcorrect
...
install_lane() {
    ...
    "$vcpkg" install "libcorrect:$triplet" --overlay-ports="$overlay/ports" --overlay-triplets="$overlay/triplets" --x-install-root="$work/installed/$lane" --x-buildtrees-root="$work/vcpkg/$lane/buildtrees" --x-packages-root="$work/vcpkg/$lane/packages" --downloads-root="$repo_root/vcpkg/downloads" >"$run_dir/logs/preparation/vcpkg-$lane.log" 2>&1
}
...
meta = {"schema_version": 2, "package": "libcorrect", "release": "snapshot ee82e667", "mechanism": "TLC Only", "workload": "four convolutional and Reed-Solomon runners"}
pathlib.Path(sys.argv[1]).write_text(json.dumps(meta, indent=2, sort_keys=True) + "\n")
summary = {"schema_version": 2, "package": "libcorrect", "status": "installed", "tests_run": False}
...
if ! $install_only; then
    LORELEI_DEVKIT="$devkit" QEMU="$qemu" WORK="$work/upstream" \
        ...
        BENCH="$overlay/ports/libcorrect/lorelei" "$recipe_dir/tests/run-installed.sh"
    mkdir -p "$run_dir/logs/upstream" "$run_dir/generated/correct" "$run_dir/generated/fec"
    cp -a "$work/upstream/results/." "$run_dir/logs/upstream/"
    cp -a "$work/upstream/dump-correct/." "$run_dir/generated/correct/"
    cp -a "$work/upstream/dump-fec/." "$run_dir/generated/fec/"
    python3 - "$work/upstream/results/summary.json" "$run_dir/summary.json" <<'PY'
import json, pathlib, sys
data = json.loads(pathlib.Path(sys.argv[1]).read_text())
data.update({"schema_version": 2, "package": "libcorrect", "tests_run": True})
