...
make_thunk() {
  ...
  "$DEVKIT/bin/LoreMakeThunk.py" --name "$name" -o "$WORK/thunk-$name" --lib "$host_lib" \
    --soname "$soname" --symbols "$dump/Symbols.conf" --desc "$desc" --devkit "$DEVKIT" \
...
for test in "${tests[@]}"; do
  ...
  run_probabilistic_test hecate "$test" env LD_LIBRARY_PATH="$DEVKIT/lib:$NATIVE_PREFIX/lib:$WORK/thunk-correct:$WORK/thunk-fec" \
    "$QEMU" -L "$DEVKIT/x86_64/sysroot" -E LD_BIND_NOW=1 \
    -E "LD_LIBRARY_PATH=$DEVKIT/x86_64/lib:$WORK/thunk-correct/x86_64:$WORK/thunk-fec/x86_64" \
