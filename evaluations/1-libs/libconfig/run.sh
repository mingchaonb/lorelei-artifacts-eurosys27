...
for pattern in "${patterns[@]}"; do
  ...
  run_logged "$run_dir/logs/preparation/thunk-$lib_name.log" "$repo_root/evaluations/1-libs/_common/lore-make-thunk.py" "$devkit/bin/LoreMakeThunk.py" --name "$lib_name" --out "$thunk" --lib "$host_lib" --symbols "$audit/Symbols.conf" --desc "$overlay_dir/ports/libconfig/lorelei/Desc.h" --gtl-alias "$(basename "$host_lib")" --no-callback-replace --devkit "$devkit" --keep-intermediates -- -I"$hecate_prefix/include"
