...
输出包括 `overall.csv`、`game-fps.csv`、`function-breakdown.csv`、`callback-track.csv`、`coverage-effort.csv` 和 `modifications.csv`。缺少 prerequisite 或 lane 时，CSV 保留状态为 `missing` 的行，不用旧论文常量填空。`manifest.json` 记录每个输入文件和 SHA-256。
...
Risotto 性能、VA/FP 和 library distribution 仍不在当前自动导出范围。
