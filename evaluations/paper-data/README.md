...
`game-fps.csv` reads the latest available MangoHud log independently for each game and each of the native, QEMU-Hecate, Box64, and Box64-Hecate lanes. It reports physical-GPU status, sample count, mean, minimum, maximum, and population variance over the fixed ten-second window from 12 seconds before the final sample up to 2 seconds before it. Game navigation remains manual, but computing the review table is automatic. A short or malformed latest log remains visible as insufficient evidence instead of being replaced with an older result.
