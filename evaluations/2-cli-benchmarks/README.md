...
The Hecate environment for Blink, Box64, and FEX follows `evaluations/3-breakdown/hecate-emulators`. `LORELEI_DEVKIT` defaults to the sibling `../lorelei-ae/build/install`. Emulator paths default to the corresponding sibling `qemu-ae`, `blink-ae`, `box64-ae`, and `FEX-ae` repositories and can be overridden with `QEMU`, `BLINK`, `BOX64`, and `FEX`.
