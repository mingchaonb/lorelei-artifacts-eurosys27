...
该 workload 使用上游 `minizip -9` 将同一份确定性 64 MiB 输入归档 5 次。port 将 `minizip`、`miniunzip` 和它们的辅助 DSO 安装到 `tools/zlib/upstream-tests`，因此 workload 只运行安装后的 artifact。计时结束后，Python ZIP reader 解压每个 member 并校验 SHA-256。
