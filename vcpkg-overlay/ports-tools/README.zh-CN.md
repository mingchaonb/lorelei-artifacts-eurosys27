...
`qemu-ae` 和 `box64-ae` 是性能评测使用的不带插桩版本。`qemu-breakdown-ae` 为直通函数调用的四阶段计时加入探针，`box64-callback-track-ae` 为 callback 地址来源 breakdown 加入探针。独立打包可以避免插桩影响普通 QEMU 与 Box64 性能数据。
