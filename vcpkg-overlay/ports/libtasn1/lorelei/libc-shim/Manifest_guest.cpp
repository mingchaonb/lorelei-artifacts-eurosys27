...
//     corrupts the host heap. Address-separation builds route untagged pointers by address range.
//     Bootstrap allocations still carry a Header so they can be recognized after startup.
//
...
//
// The errno accessor is forwarded separately so guest code observes errors set by a thunked host
// library. Allocate-and-return stdio helpers such as getline, getdelim, and asprintf remain out of
// scope because their buffers are allocated by host libc internals and carry no bootstrap tag.
...
namespace lore::thunk {
    ...
    namespace {
        ...
        // Untagged pointers can come either from guest libc internals or directly from a host
        // library. Address-separation mode gives the shared thunk runtime an exact range test, so
        // preserve guest ownership while still accepting raw host allocations returned by a
        // thunked library.
        void freeUntagged(void *p) {
            if (isHostAddress(p)) {
                ProcFn<::free, GuestToHost, Adapt>::invoke(p);
                return;
            }
            realFree(p);
        }

        void *reallocUntagged(void *p, size_t n) {
            if (isHostAddress(p)) {
                return ProcFn<::realloc, GuestToHost, Adapt>::invoke(p, n);
            }
        ...
        void *allocTagged(size_t n, size_t align, bool zero) {
            ...
            // A raw host pointer remains compatible with a host library that temporarily installs
            // a guest allocator and later restores its default allocator. The library may retain
            // such an allocation across that transition and eventually pass it straight to host
            // free(). A prefixed guest-only Header would make that otherwise valid operation fail.
            if (canForward()) {
                void *user = nullptr;
                if (align == kDefaultAlign) {
                    user = ProcFn<::malloc, GuestToHost, Adapt>::invoke(n);
                } else {
                    size_t rounded = n;
                    size_t remainder = n % align;
                    if (remainder != 0) {
                        if (n > SIZE_MAX - (align - remainder)) {
                            return nullptr;
                        }
                        rounded += align - remainder;
                    }
                    user = ProcFn<::aligned_alloc, GuestToHost, Adapt>::invoke(align, rounded);
                }
                if (user && zero) {
                    std::memset(user, 0, n);
                }
...
#ifdef LORE_QEMU_SUPPORT_ADDRESS_SEPARATION
            // Steady-state allocations are returned as raw host pointers in this mode. Classify
            // them before looking for our guest-side bootstrap Header: the bytes immediately before
            // a host allocation need not be mapped into the guest address space at all.
            if (isHostAddress(user)) {
                if (canForward()) {
                    ProcFn<::free, GuestToHost, Adapt>::invoke(user);
                }
                return;
            }
#endif
...
#ifdef LORE_QEMU_SUPPORT_ADDRESS_SEPARATION
            // As in freeTagged(), do not probe a Header before classifying a raw host pointer.
            if (isHostAddress(p)) {
                if (!canForward()) {
                    return nullptr;
                }
